		...
		try {
			...
			if (user) {
				...
				if (user.challengeName === ChallengeName.NewPasswordRequired) {
					return signIn(AuthState.ResetPassword, user);
				}
				const verifyData = await Auth.verifiedContact(user);
				if (verifyData?.verified?.email) {
					console.log('user is verified, signing them in');
					signIn(AuthState.SignedIn, user);
				}
			}
		} catch (error) {
