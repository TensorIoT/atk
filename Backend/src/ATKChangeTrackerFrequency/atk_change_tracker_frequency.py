...
def lambda_handler(event, context):
    ...
    if 'UPDATE_RATE' not in incomingBody:
        return cors_web_response(400, {'Error': 'Must provide UPDATE_RATE'})
    if 'DEVEUI' not in incomingBody:
        return cors_web_response(400, {'Error': 'Must provide DEVEUI'})
        
    update_rate = incomingBody['UPDATE_RATE']
